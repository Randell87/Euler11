...
public class Main {
    public static void main(String[] args) {
        ...
        for (int i=0; i<l; i++){
            nums.add(Character.getNumericValue(a.charAt(i)));
        }
        System.out.println("Всего элементов: " + l);
        for (int i=0; i<l; i=i+3){
            nums2.add(nums.get(i)*10 + nums.get(i+1));
        }
        System.out.println("Всего элементов: " + nums2.size());
        ...
        for(int i=0; i<16; i++){
            for(int j=0; j<16; j++){
                x = ma[i][j]*ma[i][j+1]*ma[i][j+2]*ma[i][j+3];
                if(x>max) max=x;
            }
        }
        ...
        for(int i=0; i<16; i++){
            for(int j=0; j<16; j++){
                x = ma[i][j]*ma[i+1][j]*ma[i+2][j]*ma[i+3][j];
                if(x>max) max=x;
            }
        }
        System.out.println(max);
        for(int i=0; i<16; i++){
            for(int j=0; j<16; j++){
                x = ma[i][j]*ma[i+1][j+1]*ma[i+2][j+2]*ma[i+3][j+3];
                if(x>max) max=x;
            }
        }
        System.out.println(max);
